...
if __name__ == "__main__":
    ...
    for img_name in all_imgs:
        img_nd = np.array(Image.open(img_name).resize((width, height)))
        print("img name={}, shape={}".format(img_name, img_nd.shape))
        ...
        keypoints_nd = ((keypoints_nd+1)/2 * np.array([width, height])).astype('int')
        ...
        # draw keypoints
        cv2.line(img_nd_bgr, tuple(keypoints_nd[0]), tuple(keypoints_nd[1]), [0,255,0], 1)
        cv2.line(img_nd_bgr, tuple(keypoints_nd[1]), tuple(keypoints_nd[3]), [0,255,0], 1)
        cv2.line(img_nd_bgr, tuple(keypoints_nd[3]), tuple(keypoints_nd[2]), [0,255,0], 1)
        cv2.line(img_nd_bgr, tuple(keypoints_nd[2]), tuple(keypoints_nd[0]), [0,255,0], 1)

        hm = hm1_nd[0]+hm2_nd[0]+hm3_nd[0]+hm4_nd[0]
        hm *= 255
        hm = hm.astype('uint8')
        color_heatmap = cv2.applyColorMap(hm, cv2.COLORMAP_JET)
        color_heatmap = cv2.resize(color_heatmap, (width, height))
        
        cv2.imwrite(output_dir+base_name+".jpg", cropped_image)
        cv2.imwrite(output_dir+base_name+"_raw.jpg", img_nd_bgr)
        cv2.imwrite(output_dir+base_name+"_heatmap.jpg", color_heatmap)